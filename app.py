...
if ai_contents:
    ...
    # ✨ 智能黑科技升级：制作一键切换的默认问题快捷菜单
    prompt_templates = {
        "🏥 肾移植复诊报告（专属基线版）": (
            "我是一名肾移植患者，我个人的血清肌酐（Creatinine）长期基础稳定值（Baseline）保持在 160-180 umol/L 之间。\n"
            "请帮我严格提取并整理这几页化验单中的关键信息：\n"
            "1. 肌酐（Creat）、尿素（Urea）和血红蛋白（Hb）等核心数值。\n"
            "2. 请对比我个人的基线（160-180），分析这次的肾功能是否稳定？\n"
            "3. 给出针对我当前指标需要注意的日常水分摄入、饮食和健康监测提示。"
        ),
        "🧾 车辆/商业发票收据": (
    ...
    # 渲染下拉选择框
    selected_template = st.selectbox("🔮 请选择文件类型（自动匹配最强 AI 提示词）：", list(prompt_templates.keys()))

    # 将选中的模版内容，自动塞进下面的文本输入框中，既能一键切换，又允许你手动添加新问题！
